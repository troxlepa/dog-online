...
import { getEmptyHomeField, isMyBall, isHomeField, isPartnerBall, iHaveFinished, getLastField } from "../util";
...
function Balls({gameBalls, selected, setSelected, turn, lastFour, orderMyPosition, activeCard, spectating,gameRooted, gameRules, setSnack}){
  ...
  const animatedBalls = prevSel.filter(function(element, index) { return (index % 2 === 1);  });
  ...
  function buildSuggestion(){
    ...
    if(cardval < 4 && start >= 80){ // ball in finish
      if((start%4)+cardval <= 4){ // ball field < max field
    ...
    // possibly finishes
    if(possibleMoveIntoFinish(cardval,ballColor,start)){
      var dest = 80+4*ballColor+((start+cardval)%64)-homeFields[ballColor]-1;
      if(noBallsInRange(80+4*ballColor,dest) && (dest <= 83+4*ballColor)){ // no overshoot
        out.push(dest);
      }
    }
  ...
  function movesOverBlocked(start,val){
    // moving over 0
  ...
  function possibleMoveIntoFinish(val,ballColor,start){
    if(ballColor === 0){
  ...
  function noBallsInRange(start,dest){
    return ![...Array(dest-start).keys()].map(i => start+i).some(x => gameBalls.includes(x));
  }

  function ballFinishSeven(field,selected,orderMyPosition,gameBalls){
  ...
  function moveSeven(field,selected,orderMyPosition,gameBalls){
    ...
    (field,isBall) => {
      ...
      setSelected(selected => {
        ...
        case '7':{
          ...
          const evalBalls = [...gameBalls].map((el) => {if(selected.indexOf(el) % 2 === 0){return selected[selected.indexOf(el)+1];}return el;});
  ...
  return(
    <React.Fragment>
      ...
      {gameBalls ? allFields.map((field, ix) => (
        <SvgProxy key={"f"+ix} selector={setStone(field)} onClick={() => {handleClick(field,false);}} class={"white " + (suggestion.includes(field) ? "suggested " : "") +(selected.includes(field) ? "active active"+Math.floor(selected.lastIndexOf(field)/2)+" ": " ") + (selected.length % 2 === 0 ? "" : "destination") } /> //{setColor(ix)}  {setStone(ball)}
      )) : (console.log("no entries"))}
    </React.Fragment>
  );
}
