  ...
  return(
    <React.Fragment>
      ...
      {gameBalls ? allFields.map((field, ix) => (
        <SvgProxy key={"f"+ix} selector={setStone(field)} onClick={() => {handleClick(field,false);}} class={"white " + (selected.includes(field) ? "active active"+Math.floor(selected.lastIndexOf(field)/2)+" ": " ") + (selected.length % 2 === 0 ? "" : "destination") } /> //{setColor(ix)}  {setStone(ball)}
      )) : (console.log("no entries"))}
    </React.Fragment>
  );
}
