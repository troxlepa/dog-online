      ...
      for(let j=0;j<(numCards*4);++j){
        ...
        if(!firstTurn){
          ...
          if(popp.selection){ // Thief 2 has no selection
            for(let s = 1; s < popp.selection.length; s+=2){
              if(popp.selection[s] >= 64 && popp.selection[s] < 80 && popp.selection[s] !== popp.selection[s-1]) {
                sendCount[i]++;
              }
            }
          }
...
export function computeHistory(game) {
  const history = [];
  if(game.history) {
